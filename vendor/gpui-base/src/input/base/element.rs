...
/// Visible wrapped rows with one row of overscan on either edge.
fn visible_wrapped_rows(
    ...
    let first = ((clip.top() - top).max(px(0.)) / line_height).floor() as usize;
    let end = ((clip.bottom() - top).max(px(0.)) / line_height).ceil() as usize;
    first.saturating_sub(1).min(count)..end.saturating_add(1).min(count)
}
...
impl<M: InputModeKind> TextElement<M> {
    ...
    fn calculate_visible_range(
        ...
        line_height: Pixels,
        input_height: Pixels,
        clipped_viewport: Option<Range<Pixels>>,
        ...
        let viewport_top = clipped_viewport
            .as_ref()
            .map_or((-scroll_top).max(px(0.)), |r| r.start);
        let viewport_bottom = clipped_viewport
            .as_ref()
            .map_or(viewport_top + input_height, |r| r.end);
    ...
    fn layout_lines(
        ...
        for (vi, &buffer_line) in last_layout.visible_buffer_lines.iter().enumerate() {
            ...
            let row_top = content_top
                + last_layout.line_height
                    * state.display_map.buffer_line_to_display_row(buffer_line);
            let visible_rows = visible_wrapped_rows(
                row_top,
                last_layout.line_height,
                &clip,
            ...
            let cursor = state
                .cursor()
                .saturating_sub(last_layout.visible_line_byte_offsets[vi]);
            for (row, range) in line_item.wrapped_lines.iter().enumerate() {
                // Retain geometry and byte counts for skipped rows. The first row
                // supplies continuation indentation; the caret row supplies exact x.
                if state.grow_to_content
                    && row != 0
                    && !visible_rows.contains(&row)
                    && !(range.start <= cursor && cursor <= range.end)
                {
                    wrapped_lines.push(ShapedLine::default().with_len(range.len()));
                    continue;
                }
...
impl<M: InputModeKind> Element for TextElement<M> {
        ...
        if state.grow_to_content {
            ...
            return (
                window.request_measured_layout(style, move |known, available, window, cx| {
                    ...
                    let rows = entity.update(cx, |state, cx| {
                        let (gutter, _) = Self::layout_line_numbers(
                            state,
                            &state.text,
                            text_size,
                            &text_style,
                        ...
                        state
                            .display_map
                            .set_wrapping_indent(state.wrapping_indent, cx);
                        let wrap_width = state
                            .soft_wrap
                            .then_some((width - gutter - RIGHT_MARGIN).max(px(1.)));
                        state.display_map.on_layout_changed(wrap_width, cx);
                        state.display_map.wrap_row_count().max(1)
                    });
    ...
    fn prepaint(
        ...
        let clip = window.content_mask().bounds;
        let clipped_viewport = state.grow_to_content.then(|| {
            (clip.top() - bounds.top()).max(px(0.))
                ..(clip.bottom() - bounds.top()).min(bounds.size.height)
        });
        let (visible_range, visible_buffer_lines, visible_top) =
            self.calculate_visible_range(&state, line_height, bounds.size.height, clipped_viewport);
    ...
    fn paint(
        ...
        if let Some(line_numbers) = prepaint.line_numbers.as_ref() {
                ...
                .zip(prepaint.last_layout.visible_buffer_lines.iter())
            {
                ...
                // Only the first gutter row carries a number; wrapped rows are blank.
                if let Some(line) = lines.first() {
                    let clip = window.content_mask().bounds;
                    if p.y + line_height >= clip.top() && p.y < clip.bottom() {
                        _ = line.paint(p, line_height, TextAlign::Left, None, window, cx);
                    }
                }
                offset_y += line_height * lines.len();
...
mod tests {
    #[test]
    fn virtualization_bounds_work_to_the_viewport() {
        use super::*;
        let clip = Bounds::new(point(px(0.), px(100.)), size(px(800.), px(600.)));
        assert_eq!(
            visible_wrapped_rows(px(-10000.), px(20.), &clip, 100000),
            504..536
        );
        assert_eq!(visible_wrapped_rows(px(100.), px(20.), &clip, 2), 0..2);
        assert_eq!(visible_wrapped_rows(px(-10000.), px(20.), &clip, 2), 2..2);
    }
