...
impl LineLayout {
    ...
    pub(crate) fn paint(
        ...
        let first = (((clip.top() - pos.y).max(px(0.)) / line_height).floor() as usize)
            .min(self.wrapped_lines.len());
        let end = (((clip.bottom() - pos.y).max(px(0.)) / line_height).ceil() as usize)
            .min(self.wrapped_lines.len());
        for (ix, line) in self.wrapped_lines.iter().enumerate().take(end).skip(first) {
            _ = line.paint(
...
mod tests {
    ...
    #[test]
    fn virtualization_preserves_offsets_across_unshaped_rows() {
        let layout = LineLayout::new().lines(smallvec::smallvec![
            ShapedLine::default().with_len(4096),
            ShapedLine::default().with_len(8192),
            ShapedLine::default().with_len(4),
        ]);
        let context = test_last_layout(px(20.));
        assert_eq!(layout.len(), 12292);
        assert_eq!(layout.size(px(20.)).height, px(60.));
        assert_eq!(
            layout.wrapped_line_at(point(px(0.), px(45.)), &context),
            Some((2, 12288, px(0.)))
        );
        assert_eq!(
            layout.position_for_index(12288, &context, false).unwrap().y,
            px(40.)
        );
    }
